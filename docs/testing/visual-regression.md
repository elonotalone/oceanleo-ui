...
再一条，`[实测] 2026-08-31`：这是本仓**最吃内存**的一条命令（chromium + 单 worker）。
开跑前看一眼 `free -h`，可用内存低于 ~2GB 就**排队等**，不要硬上。
当天有过一批 agent 被内存压死的记录；而 OOM 在这道闸上的表现是
**「随机某几张图截失败」**——它长得像假阳性，会把人引去查阈值和 mask，
那个方向一整天都查不出东西。所以内存要在跑之前看，不是红了之后猜。

同理，`run.sh` 的 `--ipc=host` 不是可选项：官方镜像默认的 64MB `/dev/shm`
会让 chromium 崩溃，而崩溃的表现同样是「随机某几张图截失败」。
...
值得写清楚，因为它是整个模块解析的机关，改错了会很难查。

官方镜像给了浏览器（`/ms-playwright` 下 chromium-1228 等），**没给** `@playwright/test`。
`Dockerfile` 把它装在 `/pw`——一个与被测仓完全无关的前缀——再软链
`/pw/node_modules → /node_modules`。node 的模块解析从文件所在目录逐级向上找：
...
必须能找到），但**没有** `@playwright/test`（它只在 `package.json` 里声明）。
于是解析自然穿过 `/work/node_modules`，落到镜像的 `/node_modules` 上。

⇒ **被测仓的包从宿主树取，runner 的包从镜像取**，两边不互相污染，
...
**这一节和上一节是一对，但它更要紧。**
假红烦人，可至少有人会去追。**假绿没有人会去查**——报表是绿的，
所有人都以为这件事被守着，而它从来没被守过。

这道闸开张十天里，四种假绿全部真实出现过或差一步就发生。
写在手册里而不是只当 bug 修掉，是因为**每一种都对应一条必须长期留着的结构性规矩**：
修补会被下一个人改回去，规矩不会。

### 1. 占位符被写成基线

`--update-snapshots` 会给它看到的**任何东西**建基线，包括夹具页自己的占位块。

`needs-client` 主体在夹具页上渲染的是琥珀色的 `[data-leo-needs-client]` 占位块。
而 `expectSubjectPresent()` 只查「导出还在不在」，占位块照样通过。
于是 `-u` 一跑，占位块被逐字节写成基线，此后这条用例**永远绿**——
绿的是「占位符还长得跟上次一样」。

**这比「进程没跑却报成功」更难发现**：它真的跑了、真的生成了 png、真的会绿，
报表上没有任何异样。等于闸自己给自己盖了章。

`[实测] 2026-08-31` 差一步就发生：`w05` 与 `w07` 两条截图用例都缺 coverable 那一行，
而两者当天都是 `needs-client`。是在生成之前抓住的，盘上没有留下假基线。

⇒ **规矩：凡 `toHaveScreenshot` 之前必有 `expectSubjectCoverable`。**
今天是 `ssr` 的主体（`w04` / `w08`）也要写上这一行，虽然它此刻空过。
哪天有人把主体改成 `needs-client`，它会当场红，而不是悄悄换成一张占位符基线。

### 2. 退化状态被写成基线（同一种病，换了个样子）

上一条的规矩挡的是**占位块**。但「已知的退化状态」不止占位块一种，
`[实测] 2026-08-31` 在 `w01` 上抓到了第二种，而且当时口子真的开着。

`motionTokens` 是 `cssOnly` 主体，夹具渲染的是 token 探针块，
永远不出占位符 ⇒ 第 1 条那道判据在这一组**空转**。
可 `ui.css` 陈旧时六档会全部解析成 `0s`
（不是空串：`transition-duration: var(--未定义)` 在计算值阶段失效，回落到初始值），
探针块于是渲染成一排「没有任何时长」的方块，`-u` 把这排方块写成基线 ⇒ 永远绿。

⇒ **规矩：截图之前先断言被测对象处在「有效状态」，而不只是「存在」。**
判断问自己一句：**这张图现在长这样，是因为东西对，还是因为东西坏了但坏得很稳定？**
稳定的坏也会逐字节相等，也会绿。

`w01` 现在的做法是截图前要求六档解析值 > 0；不过就红在那里，进不了 `toHaveScreenshot`。

### 3. 判据量了个空

`[实测] 2026-08-31` `w04` 的 focus-visible 反面判据：
「鼠标点击后不该命中 `:focus-visible`」。
若选择器失效、点了个空，那么**没聚焦的元素当然不命中** ⇒ 判据恒真 ⇒ 恒绿。

一切形如「X 不该发生」的判据都有这个口子：
**X 没发生，可能是因为压根什么都没发生。**

⇒ **规矩：反面判据必须先断言前置条件真的成立。**
那条用例现在先断 `document.activeElement === node`，再断它不出焦点环。

（同一条用例还踩过另一个坑，属于假红那一类，记在这里免得后人重犯：
反面判据点的必须是**另一枚**按钮。点上一步刚 Tab 聚焦的那枚，焦点不移动，
浏览器不重算焦点环状态，键盘留下的 `:focus-visible` 会原样保留 ⇒ 正确实现也必红。）

### 4. 进程没跑却报成功

`[实测]` 曾留下一份 `results.json`：
`{"expected":0,"skipped":38,"unexpected":0,"duration":1634}`，且 `errors: []`。
38 例全 skip、零 error、1.6 秒——这是**进程在第一个用例开跑前被中断**的形状，
不是判据结果。而套件里根本没有任何 skip 逻辑
（`rg -n "skip|fixme" tests/visual` 只命中注释文字）。

⇒ **规矩：读数必须连 skip 一起读，四个数一起写。**
「0 红」不等于绿。`绿 + 红 + skip` 对不上执行例数，就是没跑完。
本手册与交付说明里所有读数都写成
**「N 例执行 / N 绿 / N 红 / N skip」**，就是为了让这种形状一眼露馅。

### review 侧的对应动作

基线是提交物，所以最后一道防线在 review：

- diff 里出现 `.png` 而提交信息没提为什么变 ⇒ **打回**；
- 新增基线图**打开看一眼**：琥珀色方框 = 占位符被写成基线；
  一排没有时长/没有样式的裸元素 = 退化状态被写成基线。两者都打回；
- 读数只给「多少红」不给 skip 数 ⇒ 要求补全四个数再看。

### 基线报废条件

改了下面任何一项，**盘上全部基线立即作废**，必须在容器里整套重新生成，
并在提交信息里写明是哪一项变了：

| 改动 | 为什么报废 |
|---|---|
| `Dockerfile` 的 `FROM` / `run.sh` 的 `BASE_IMAGE` | 换 driver = 换渲染路径 |
| `playwright.config.ts` 的 `viewport` | 布局全变 |
| `deviceScaleFactor` | 每一个像素都变 |
| `page-template.mjs` 的 `FONT_STACK` 或夹具版面 CSS | 文字占了基线图绝大多数像素 |
| `harness.js` 的 `MASK_SELECTORS` | mask 区域变了，图就跟着变 |

前两项 `run.sh` 已经硬防了一半（两处镜像字符串必须逐字一致），其余靠这张表。
...
这条是踩过才写的，`[实测] 2026-08-31`。

登记表原先把 `button` 指着 `src/ui/index.tsx`，依据是台账
「`src/ui/index.tsx` 没有 `Button`」。那句话**今天仍然是对的**，
但由它推出的「`Button` 不存在」**是错的**：W04 已经交卷，原语落在
`src/ui/Button.tsx:250`，只是没有从桶文件再导出。

照原样跑，这道闸会对**唯一一个已经把主体交出来的 owner 报假红**。

两条教训，对后来接手的人都成立：

- **「A 文件里没有」不等于「仓里没有」。** 判「不存在」要看 AST 或直接看导出声明，
  不要靠一句 grep 结论，更不要靠一句**别人写的、可能已经过期的** grep 结论。
- **台账里的事实会过期。** 这道闸守的九份改动与它本身是同一波并发造的，
  所以「开工时零命中」这类记录半小时后就可能不成立。红之前重核一次，一分钟的事。
