...
test("共享包不许再持有旧的按键映射（配额凑出来的那份 2177 枚）", () => {
  // 旧口径:「L4 站级装配单点了这个族 × 该族有编辑器实现 ⇒ 发按钮」。前者是一张按 12 个
  // 货架位次凑数的素材备货配额单,后者只说明这个族有编辑器实现,两个条件都与「这个 app
  // 的用户需不需要这件工具」无关。操作员 2026-08-05 把授权换成 W10 手写的清册
  // （一件工具问一次「哪些 app 需要它」,每条都要写理由）。
  //
  // 这条门禁盯的是**回潮**:只要包里还留着旧数据模块或旧 schema 的读取点,下一个人就能
  // 把 2177 枚按钮原样接回来,而两套数据并存时谁也说不清按钮到底从哪来。
  const offenders = [];
  for (const path of sourceFiles()) {
    const rel = path.slice(SRC.length);
    const src = stripComments(readFileSync(path, "utf8"));
    if (src.includes("app-capability-map-generated")) {
      offenders.push(`${rel}（还在 import 已删除的旧生成物）`);
    }
    if (src.includes("oceanleo.app-capability-map.v1")) {
      offenders.push(`${rel}（还在认旧 schema）`);
    }
  }
  assert.deepEqual(
    offenders,
