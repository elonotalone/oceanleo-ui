...
test("G4: 同一 handoff 的重新解析不卸载 PPTist，也不重发稿件", async () => {
  ...
  try {
    ...
    const initialMessages = first.messages.filter((message) => message.type === "open-asset");
    ...
    assert.equal(
      first.messages.filter((message) => message.type === "open-asset").length,
      1,
...
test("G4: 内容 revision 变化时新 iframe 等自己的 ready 后才收稿", async () => {
  ...
  try {
    ...
    assert.equal(second.messages.filter((message) => message.type === "open-asset").length, 1);
  } finally {
    await view.unmount();
  }
});
