...
test("document, grid and deck chains expose load-edit-save-export without globals", () => {
  ...
  // 表格只剩 Univer 舞台（core-swap:delete grid）：载入 → Univer 内编辑 → 存盘 → 导出。
  ...
  assert.match(grid, /editor_project_url: saved\.projectUrl/);
...
test("lightweight stages announce loading, error and empty states accessibly", () => {
  ...
  // grid 的自研舞台已删：Univer 舞台首帧用 WorkbenchRouteLoading（role="status"）盖住画布。
  const gridStage = source("../src/shell/doc-editors/GridUniverStage.tsx");
  ...
  for (const stage of [richStage, deckStage]) {
    assert.match(stage, /aria-busy=\{editor\.loading\}/);
    assert.match(stage, /role="status"/);
    assert.match(stage, /role="alert"/);
  }
  assert.match(gridStage, /\{!snapshotReady && \(/);
  assert.match(gridStage, /<WorkbenchRouteLoading \/>/);
  assert.match(gridStage, /data-grid-univer-loading="true"/);
