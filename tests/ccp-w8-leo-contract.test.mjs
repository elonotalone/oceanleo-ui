...
const transcript = readFileSync(new URL("../src/shell/leo/LeoTranscript.tsx", import.meta.url), "utf8");
...
test("W8 composer 保持可见的多行输入区", () => {
  ...
  assert.match(transcript, /data-leo-transcript/);
  assert.match(assistant, /flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden/);
});
