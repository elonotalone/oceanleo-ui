...
在这个目录建起来之前，全平台的按键共用五份通用空白起手件（`../blank-draft-items.ts`），
点地图和点换算器落到的是同一份「输入 A / 输入 B / 比例」。那条路已经整条拆掉；
现在每个插件的第一屏由这里逐个给出，查不到就是查不到，不回退。
...
`tests/plugin-initial-states.test.mjs` 里的名单**全是派生的**，没有一份手抄的清单要你
同步维护（手抄名单过期正是这份测试前两次变红的原因）。派生出来的对账有三个方向：

- 清册发了按键、这里没有第一屏 → 红（那就是用户点下去打不开的死按键）；
- 这里有第一屏、清册没发按键也没登记留空 → 红（做完了没人用，或漏了登记）；
- 清册登记为留空、这里却已经做了 → 红（登记过期）。

所以补一件新工具是两步：第一屏落进本目录，清册那一侧给它授权。缺哪一步都会红在
具体是哪一种缺，不会静默通过。

运行期还有一道兜底：`ResultCanvas.startFeatureLaunch()` 打开之前问一次
`pluginInitialStateAvailable()`，没有第一屏就显示「缺什么、下一步做什么」，
不挂空壳。路由层刻意不设这道门槛——用户存下来的实例即使第一屏后来被撤掉也必须打得开。
