  ...
  return (
    <section className="space-y-2.5 border-t border-[var(--border,#e7e5e4)] pt-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {style.background && (
              <button
                ...
                className="rounded-lg border border-[var(--border,#e7e5e4)] bg-[var(--card,#fff)] px-2 py-1.5 text-[11px] text-[var(--fg-2,#57534e)]"
              >
                {tt("去底色")}
              </button>
            )}
      ...
      {(track.kind === "video" || track.kind === "image") && (
        <>
          <p className="border-t border-[var(--border,#e7e5e4)] pt-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--muted,#78716c)]">
            {tt("画面变换")}
          </p>
          {track.kind === "video" && (
            <>
      ...
      {/* 转场（与前一 clip 之间） */}
      <div className="border-t border-[var(--border,#e7e5e4)] pt-2.5">
