    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-8">
        <div className="relative mx-auto min-h-full max-w-[860px] rounded-xl border border-[var(--border,#e7e5e4)] bg-[var(--card,#fff)] px-8 py-10 shadow-sm sm:px-14">
          ...
          {!editor.loading && !editor.sourceFailed && editor.error && (
            <div
              role="alert"
              className="absolute inset-x-6 top-4 z-20 rounded-lg border border-[color-mix(in_srgb,var(--awb-danger)_35%,transparent)] bg-[var(--awb-danger-soft)] px-3 py-2 text-[11px] text-[var(--awb-danger)]"
            >
              {tt(editor.error)}
            </div>
          )}
      ...
      <div className="flex h-8 shrink-0 items-center gap-2 border-t border-[var(--border,#e7e5e4)] bg-[var(--card,#fff)] px-4">
        ...
        {!editor.sourceFailed && editor.error && (
          <span
            role="alert"
            className="min-w-[120px] flex-1 truncate text-[10px] text-[var(--awb-danger)]"
          >
            {tt(editor.error)}
          </span>
        )}
